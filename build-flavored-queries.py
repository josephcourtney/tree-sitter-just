...
@dataclass
class Flavor:
    """Representation of a single flavor."""

    # Display name of the flavor
    name: str
    # Tag used
    tag: str
    # Regex replacements to perform on the queries
    replacements: list[tuple[str, str]]
    # Not all query flavors support all captures, so we check against an allowlist.
    allowed_captures: dict[str, list[str]]
    # Not all query flavors support all settings, so we check against an allowlist.
    allowed_settings: dict[str, list[str]]
    # Where to generate this flavor's queries to.
    base_path: Path
...
def main():
    ...
    for fname in sources:
        ...
        for fm in FLAVOR_MAPPINGS:
            ...
            fm.base_path.mkdir(parents=True, exist_ok=True)
            dest = fm.base_path / Path(fname).name
